...
def lin_comb_mat_vec_mult(M, v):
    ...
    >>> lin_comb_mat_vec_mult(M, v) == Vec({0, 1, 2},{0: 1, 1: 5, 2: 6})
    True
    >>> A = Mat(({'a', 'b'}, {'@', '#', '?'}),
    ... {('a', '@'): 2, ('a', '#'): 1, ('a', '?'): 3, ('b', '@'): 20,
    ... ('b', '#'): 10, ('b', '?'): 30})
    >>> b = Vec({'@', '#', '?'}, {'@': 0.5, '#': 5, '?': -1})
    >>> lin_comb_mat_vec_mult(A, b) == Vec({'a', 'b'}, {'a': 3, 'b': 30.0})
    True
    '''
    assert v.D == M.D[1]
    rd = mat2rowdict(M)
...
def lin_comb_vec_mat_mult(v, M):
    ...
    >>> lin_comb_vec_mat_mult(v3, M2) == Vec({0, 1},{0: -40, 1: 80})
    True
    '''
    assert v.D == M.D[0]
    cd = mat2coldict(M)
...
if __name__ == '__main__':
    ...
    M2 = listlist2mat([[-5, 10], [-4, 8], [-3, 6], [-2, 4]])
    v3 = Vec({0, 1, 2, 3}, {0: 4, 1: 3, 2: 2, 3: 1})
    v4 = lin_comb_vec_mat_mult(v3, M2)
